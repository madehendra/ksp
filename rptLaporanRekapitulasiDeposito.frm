...
Begin VB.Form rptLaporanRekapitulasiDeposito 
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame3 
      ...
      Begin BiSAButtonProject.BiSAButton cmdKeluar 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptLaporanRekapitulasiDeposito.frx":0000
      End
      Begin BiSAButtonProject.BiSAButton cmdPreview 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptLaporanRekapitulasiDeposito.frx":00A6
      End
      Begin BiSAButtonProject.BiSAButton cmdOK 
         ...
         Alignment       =   1
         BackColor       =   -2147483633
         Picture         =   "rptLaporanRekapitulasiDeposito.frx":032C
      End
   End
   ...
   Begin BiSAFramProject.BiSAFrame BiSAFrame2 
      ...
      Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
         ...
         Columns(0).Caption=   "PDL"
         Columns(0).DataField=   "Nama"
         Columns(0)._PropDict=   "_MaxComboItems,516,2;_VlistStyle,514,3"
         Columns(1)._VlistStyle=   0
         Columns(1)._MaxComboItems=   5
         Columns(1).Caption=   "Nama PDL"
   ...
   End
End
Attribute VB_Name = "rptLaporanRekapitulasiDeposito"
...
Dim nTmpResi As Double
  ...
  GetUpdateGrid
  Set dbData = objData.Browse(GetDSN, "PDL p", "p.Kode,p.Keterangan,count(t.Rekening) as Nasabah", , , , "1=1 Group by p.kode", "p.Kode", Array("Left Join Deposito t on t.PDL = p.Kode"))
  If Not dbData.eof Then
    ...
    Do While Not dbData.eof
      ...
      vaArray(n, 2) = GetResi(objData, vaArray(n, 0), trDeposito.trPembukaan)
      vaArray(n, 3) = GetNull(dbData!Nasabah)
      vaArray(n, 4) = GetSaldoLalu(objData, vaArray(n, 0))
      vaArray(n, 5) = GetSaldoMutasi(objData, vaArray(n, 0), trDeposito.trPembukaan)
      vaArray(n, 6) = GetSaldoMutasi(objData, vaArray(n, 0), trDeposito.trPencairanPokok)
...
Dim db As New ADODB.Recordset
  ...
  GetSaldoLalu = 0
  Set db = obj.Browse(GetDSN, "MutasiDeposito m", "m.Rekening,m.Jumlah,m.KodeMutasi", "m.tgl", sisLT, Format(dTgl(0).Value, "yyyy-MM-dd"), " and p.Kode ='" & PDL & "' and (m.KodeMutasi='" & trDeposito.trPembukaan & "' or m.KodeMutasi='" & trDeposito.trPencairanPokok & "')", , Array("Left Join Deposito t on t.Rekening = m.Rekening", "Left Join PDL p on p.Kode = t.PDL"))
  If Not db.eof Then
    FrmPB.InitPB db.RecordCount
    Do While Not db.eof
      FrmPB.RunPB
      GetSaldoLalu = GetSaldoLalu + IIf((db!KodeMutasi) = "2", -(db!Jumlah), (db!Jumlah))
      db.MoveNext
    Loop
    FrmPB.EndPB
  End If
...
Dim db As New ADODB.Recordset
  ...
  GetSaldoMutasi = 0
  Set db = obj.Browse(GetDSN, "MutasiDeposito m", "m.Rekening,m.Jumlah,m.KodeMutasi", "m.tgl", sisLTEqual, Format(dTgl(1).Value, "yyyy-MM-dd"), " and m.tgl >='" & Format(dTgl(0).Value, "yyyy-MM-dd") & "' and p.Kode ='" & PDL & "' and m.KodeMutasi='" & KodeTransaksi & "'", , Array("Left Join Deposito t on t.Rekening = m.Rekening", "Left Join PDL p on p.Kode = t.PDL"))
  If Not db.eof Then
    FrmPB.InitPB db.RecordCount
    Do While Not db.eof
      FrmPB.RunPB
...
Dim db As New ADODB.Recordset
  ...
  GetResi = 0
  Set db = obj.Browse(GetDSN, "MutasiDeposito m", "count(m.Rekening) as Resi", "m.tgl", sisGTEqual, Format(dTgl(0).Value, "yyyy-MM-dd"), " and m.Tgl <= '" & Format(dTgl(1).Value, "yyyy-MM-dd") & "' and p.Kode ='" & PDL & "' and m.KodeMutasi = '" & KodeTransaksi & "'", , Array("Left Join Deposito t on t.Rekening = m.Rekening", "Left Join PDL p on p.Kode = t.PDL"))
  If Not db.eof Then
    FrmPB.InitPB db.RecordCount
    Do While Not db.eof
      FrmPB.RunPB
...
Private Sub rpt()
  With FrmRPT
    .AddPageHeader UCase("Laporan Rekapitulasi Keseluruhan Deposito PDL"), tdbHalignCenter, , , , , 14, True, True
